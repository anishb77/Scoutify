...
/* ------------------------------------------------------------------ */
/*  DATA LAYER — replace with your FastAPI backend                    */
/*                                                                     */
/*  This block is a skeleton: it shapes the request the same way      */
/*  scoutify.py's package_scouting_data() expects, and shapes the     */
/*  mock response the same way that function returns. Nothing below   */
/*  this block needs to change once you wire up the real endpoint —   */
/*  just replace the body of fetchScoutingReport().                   */
/* ------------------------------------------------------------------ */
...
// Same five bands as feedback() in scoutify.py.
...
/* ---- everything below this line is placeholder-only mock logic ---- */
/* ---- it stands in for the real model until FastAPI is wired up ---- */

const MOCK_RANGES = {
  "40yd": { min: 4.24, max: 5.6, inverse: true },
  "Vertical": { min: 24, max: 46, inverse: false },
  "Broad Jump": { min: 96, max: 147, inverse: false },
  "Bench": { min: 0, max: 40, inverse: false },
  "3Cone": { min: 6.28, max: 8.3, inverse: true },
  "Shuttle": { min: 3.78, max: 4.9, inverse: true },
};

function mockNormalize(key, value) {
  const { min, max, inverse } = MOCK_RANGES[key];
  const t = inverse ? (max - value) / (max - min) : (value - min) / (max - min);
  return clamp(t, 0, 1);
}

function mockGrade(key, value) {
  const t = mockNormalize(key, value);
  return Math.round(10 + Math.pow(t, 0.5) * 90);
}

function mockGrades(stats) {
  const speed = "40yd" in stats ? mockGrade("40yd", stats["40yd"]) : 10;
  const power = "Bench" in stats ? mockGrade("Bench", stats["Bench"]) : 10;
  const explosiveness =
    "Vertical" in stats && "Broad Jump" in stats
      ? Math.round((mockGrade("Vertical", stats["Vertical"]) + mockGrade("Broad Jump", stats["Broad Jump"])) / 2)
      : 10;
  const agility =
    "Shuttle" in stats && "3Cone" in stats
      ? Math.round((mockGrade("Shuttle", stats["Shuttle"]) + mockGrade("3Cone", stats["3Cone"])) / 2)
      : 10;
  return { speed, power, explosiveness, agility };
}

// Fictional names standing in for similar_grades()'s nearest-neighbor
// lookup against the real draft-class dataframe.
const NAME_POOL = [
  "M. Whitfield", "R. Castellan", "D. Okafor", "T. Vasquez", "J. Amaro",
  "K. Devereux", "S. Braddock", "L. Fenwick", "C. Deshields", "A. Marchetti",
  "P. Ojeda", "N. Sarkozy", "B. Holloway", "G. Talvik", "W. Prosser",
  "H. Duquette", "F. Osei", "V. Kanaris", "O. Brandveld", "Q. Ibarra",
];

function generateComparisons(grades, n) {
  const shuffled = [...NAME_POOL].sort(() => Math.random() - 0.5).slice(0, n);
  const jitter = () => Math.round(Math.random() * 24 - 12);
  const comps = shuffled.map((name) => ({
    Player: name,
    speed: clamp(grades.speed + jitter(), 10, 100),
    power: clamp(grades.power + jitter(), 10, 100),
    explosiveness: clamp(grades.explosiveness + jitter(), 10, 100),
    agility: clamp(grades.agility + jitter(), 10, 100),
  }));
  const dist = (c) =>
    Math.sqrt(GRADE_KEYS.reduce((acc, k) => acc + (c[k] - grades[k]) ** 2, 0));
  return comps.sort((a, b) => dist(a) - dist(b));
}
 ...
 * FastAPI route wrapping package_scouting_data():
 *
 ...
 *
 * The resolved value should keep this shape (same keys
 * package_scouting_data() returns in scoutify.py):
 *   {
 *     position: string,
 *     input_stats: {...},
 *     grades: { speed, power, explosiveness, agility },     // 10-100
 *     feedback: { speed, power, explosiveness, agility },   // tier strings
  ...
  return (
    <div className="scoutify-root">
      <style>{`
        ...
        .submit-row { display: flex; justify-content: flex-end; margin-top: 32px; }
        .submit-btn {
          font-family: 'Courier Prime', monospace;
          font-weight: 700;
          font-size: 14px;
      ...
      <div className="shell">
        ...
        {stage === "results" && report && (
          <div ref={resultsRef}>
            ...
            {/* Roadmap — feedback tier per pillar */}
            <div className="panel-outer">
              <div className="panel-tab">Development Roadmap</div>
              <div className="panel">
                <div className="panel-hint">Grade tiers</div>
                <div className="roadmap-list">
                  {GRADE_KEYS.map((k) => {
                    const grade = report.grades[k];
                    const tier = report.feedback[k];
                    const pct = clamp(((grade - 10) / 90) * 100, 0, 100);
                    return (
                      <div className="roadmap-row" key={k}>
                        ...
                        <div className="roadmap-track">
                          <div className="roadmap-marker" style={{ left: `${pct}%` }}>{grade}</div>
                        </div>
        ...
        <div className="disclaimer">
          <b>Note:</b> comp names and grades above are sample data standing
          in for scoutify.py's real output — point fetchScoutingReport() in
          this file at your FastAPI route wrapping package_scouting_data()
          to power live comparisons.
        </div>
      </div>
    </div>
  );
